...
raw_data['timestamp'] = pd.to_datetime(raw_data['timestamp'], format='mixed', yearfirst=True, utc=True)
...
raw_data['humidity'] = pd.to_numeric(raw_data['humidity'], errors='coerce')
...
st.sidebar.selectbox("Select Beehive ID",options=beehive_ids, key="selected_beehive_id")
...
agg_data_selected_beehive = agg_data[agg_data['beehive_id'] == st.session_state.selected_beehive_id]
with col1:
    calculate_and_display_metric(agg_data_selected_beehive,'weight', 'Weight (kg)',start_date_input,end_date_input, col1)
    plot_line_chart(agg_data_selected_beehive,'weight','green')
with col2:
    calculate_and_display_metric(agg_data_selected_beehive,'temperature', 'Temp. (°C)',start_date_input,end_date_input, col2)
    plot_line_chart(agg_data_selected_beehive,'temperature','red')
with col3:
    calculate_and_display_metric(agg_data_selected_beehive,'humidity', 'Humidity (%)',start_date_input,end_date_input, col3)
    plot_line_chart(agg_data_selected_beehive,'humidity','blue')

# Create different tabs
tab1, tab2, tab3= st.tabs(["Raw Data","Beehive comparison", "Rapid Weight Changes"])
with tab1:
    raw_data_date_range_selected_beehive= raw_data_date_range[raw_data_date_range['beehive_id'] == st.session_state.selected_beehive_id]
    st.write(
        f"#### Raw Data: {', '.join([col.capitalize() for col in columns_to_plot])} over the last 24 hours")
    st.write(f"<span style='color: grey;'>Latest timestamp: {raw_data['timestamp'].max().strftime('%H:%M %d-%m-%Y')}</span>", unsafe_allow_html=True)
...
with tab2:
    ...
    random_beehive_id = random.choice([beehive_id for beehive_id in beehive_ids if beehive_id != st.session_state.selected_beehive_id])
